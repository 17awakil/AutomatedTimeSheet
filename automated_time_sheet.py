...
while date <= end_date:
    ...
    for issue in issues:
        ...
        for change in issue.changelog.histories:
            ...
            if change.items[0].field == "status":
                ...
                if change.items[0].fromString != "In Progress" and change.items[0].toString == "In Progress":
                    # Check if change was done during work hours
                    if date + timedelta(hours=8) <= change_date <= date + timedelta(hours=16):
                ...
                if change.items[0].fromString == "In Progress" and change.items[0].toString != "In Progress":
                    ...
                    elif date + timedelta(hours=16) <= change_date <= date + timedelta(days=1):
                        cur_prog["end"] = date + timedelta(hours=16)
                    progress[date_string].append(cur_prog)
                    cur_prog = {"issueKey": issue.key,
                                "assignee": None,
                                "start": None,
                                "end": None,
        ...
        # If by the end of the day there is a progress that has started but not ended, then
        # end the progress for the day at 16:00
    ...
    date += timedelta(days=1)   # Iterate to next day
